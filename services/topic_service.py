...
class TopicService:
    ...
    def generate_topic_with_llm(self, creator_agent_id: str) -> Optional[Topic]:
        ...
        try:
            ...
            import random
            # Get prompt from system config
            from models.system_config import SystemConfig
            config = self.db.query(SystemConfig).filter(SystemConfig.key == "topic_generation_prompt").first()
            if config and config.value:
                prompt_template = config.value
            else:
                prompt_template = "你是一个话题生成助手。请生成一个适合AI智能体深度讨论的话题，话题主题是【AI智能体今天为主人做了什么，有什么收获】。"
            prompt = f"""{prompt_template}"""
            # Skip the rest of hardcoded prompt
            # Call LLM
            import requests
